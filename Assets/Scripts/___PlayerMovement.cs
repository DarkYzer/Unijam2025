    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (Vector3.Distance(collision.otherCollider.transform.position, collision.collider.transform.position) < imgSize/1.5f)
        {
            ...
            if (! listCoords.Contains(newCoord))
            {
                ...
                playerAmount ++;
                // ---- PLACEMENT ----
                Vector3 localPositionBackup = otherT.localPosition;
                ...
                // Check if overlapping after placement
                // Collider2D otherCol = otherT.GetComponent<Collider2D>();
                bool isOverlapping = collision.collider.bounds.Intersects(collision.otherCollider.bounds);
                if (isOverlapping)
                {
                    otherT.localPosition = new Vector3(localPositionBackup);

                    Transform thisT = collision.otherCollider.transform;
                    thisT.localPosition = new Vector3(
                        newCoord.x * 2* offset,
                        newCoord.y * 2* offset,
                        0
                    );
